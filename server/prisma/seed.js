// Idempotent seed: one admin user + the launch press release.
// Run with: npx prisma db seed   (also runs automatically after `migrate reset`)
...
async function main() {
    // Admin user (idempotent, parameterized)
    ...
    // Seed post (idempotent by slug; gallery replaced atomically)
    await prisma.$transaction(async (tx) => {
        ...
        await tx.galleryImage.createMany({
            data: GALLERY.map((g) => ({ ...g, postId: post.id })),
        });
    });
