...
- ~~The stored admin password hash is at bcrypt cost 10~~ — **resolved 28 July 2026.** The row was rotated with `RESET_ADMIN_PASSWORD=1` and now reads `$2a$12$`, so both login branches do equal work and the username-enumeration timing oracle is closed. History, because the failure mode is subtle and will recur if the cost is ever raised again: `bcryptCost.test.ts` only proves the two *code* constants agree — it cannot see the row in Neon. That row predated the cost bump, so the real-user branch compared against a cost-10 hash while a missing user compared against the cost-12 `DUMMY_HASH`, which **inverted** the oracle rather than closing it: measured on the preview against real data, interleaved, n=12 each, an existing username answered in **371 ms** and an unknown one in **703 ms** — a valid username identified by being *faster*.

  **Two defences now, keep both.** `login.ts` re-hashes the stored password at `BCRYPT_COST` after a verified sign-in whenever the row's cost is lower (`needsRehash` in `src/lib/bcryptHash.ts`), so a future cost bump migrates itself on the next login; the upgrade is wrapped so it can never fail a valid sign-in, and it only ever upgrades, so lowering `BCRYPT_COST` cannot weaken stored credentials. Verify the stored cost any time with the read-only one-liner above — you want `$2a$12$`.

  **The admin password is deliberately short** (owner's decision, recorded so nobody "fixes" it as a bug). The Firewall rule below is therefore the primary defence against online guessing, not a secondary one — do not remove or loosen it, and if it is ever disabled, rotate to a strong passphrase first.
